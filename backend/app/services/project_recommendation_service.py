from app.data.career_mapping import CAREER_MAPPING
from app.data.project_catalog import PROJECT_CATALOG


DIFFICULTY_ORDER = {
    "Beginner": 1,
    "Intermediate": 2,
    "Advanced": 3,
}


class ProjectRecommendationService:
    """
    Service responsible for recommending projects based on
    the user's career path, missing skills, and difficulty level.
    """

    def recommend_projects(
        self,
        career: str | None,
        missing_skills: list[str],
        difficulty: str | None = None,
    ) -> list[dict]:
        """
        Recommend projects that best match the user's
        career path and missing skills.

        Parameters
        ----------
        career:
            Recommended career path.

        missing_skills:
            Skills the user needs to improve.

        difficulty:
            Optional difficulty filter
            (Beginner, Intermediate, Advanced).

        Returns
        -------
        list[dict]
            A list of recommended projects sorted by
            relevance and difficulty.
        """

        recommendations = []

        # ----------------------------------------
        # Resolve career mapping
        # ----------------------------------------
        if career:
            careers = CAREER_MAPPING.get(
                career,
                [career],
            )
        else:
            careers = []

        # ----------------------------------------
        # Normalize missing skills
        # ----------------------------------------
        normalized_missing_skills = {
            skill.strip().lower()
            for skill in missing_skills
        }

        # ----------------------------------------
        # Recommend projects
        # ----------------------------------------
        for project in PROJECT_CATALOG:

            # Filter by mapped careers
            if careers:
                if project["career"] not in careers:
                    continue

            # Filter by difficulty
            if (
                difficulty
                and project["difficulty"].lower()
                != difficulty.lower()
            ):
                continue

            # Find matching skills
            matched_skills = [
                skill
                for skill in project["skills"]
                if skill.lower() in normalized_missing_skills
            ]

            recommendations.append(
                {
                    "title": project["title"],
                    "career": project["career"],
                    "difficulty": project["difficulty"],
                    "description": project["description"],
                    "skills": project["skills"],
                    "matched_skills": matched_skills,
                    "match_score": len(matched_skills),
                }
            )

        # ----------------------------------------
        # Sort recommendations
        # ----------------------------------------
        recommendations.sort(
            key=lambda project: (
                -project["match_score"],
                DIFFICULTY_ORDER.get(
                    project["difficulty"],
                    999,
                ),
            )
        )

        return recommendations